...
def main():
    ...
    try:
        frame_count = 0 #frame 세기
        prev_distances = {}  #초기 dist 값 (객체별 딕셔너리)
        danger_levels = {}

        while True:
            frame_count += 1 #frame 세기
            ...
            for (x1m,y1m,x2m,y2m,cls,conf) in dets: #원본 카메라 좌표로 돌리기
                ...
                if cx < screen_center_x :  # 30픽셀 정도의 중앙 영역(deadzone) 설정
                    position_str = "Right" #자전거 탑승자 기준
                else :
                    position_str = "Left"
                # --------------좌우 판별
                ...
                #ttc 부분 ----------------------------
                danger=0
                if frame_count % 3 == 0 and dist_m > 0: # 0.1초마다, 거리가 유효할 때
                    if cls in prev_distances:
                        prev_dist = prev_distances[cls] 
                        ddist = prev_dist - dist_m# 거리 변화량

                        if ddist > 0: # 다가오는 경우에만 TTC 계산
                            relative_speed = ddist * 10.0 # m/s 단위 (0.1초간의 변화량이므로 *10)
                            if relative_speed > 0.1: # 아주 작은 움직임은 무시
                                ttc = dist_m / relative_speed
                    ...
                    prev_distances[cls] = dist_m # 현재 거리를 다음 계산을 위해 업데이트
                
                if cls in danger_levels:
                    danger = danger_levels[cls]
                
                danger_levels[cls] = danger # 계산된 위험도를 딕셔너리에 저장   
                #--------------------------------ttc 부분
                
                label = f"id{cls} {dist_m:.2f} m [{position_str}"
                
                cv2.rectangle(vis, (x1,y1), (x2,y2), (0,255,0), 2) #화면에 그리기
                cv2.putText(vis, label, (x1, max(0,y1-7)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
                
                print(f"[detections: {cls} {dist_m:.3f}m {position_str}] ")
             #----------------의준 코드 반영
